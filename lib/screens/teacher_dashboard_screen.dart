...
class TeacherDashboardScreen extends StatelessWidget {
  ...
  Widget _buildTeacherHeader() {
    return Container(
      ...
      child: Stack(
        children: [
          Positioned(right: -30, top: -30, child: Container(width: 150, height: 150, decoration: BoxDecoration(shape: BoxShape.circle, color: AppColors.accentGreen.withOpacity(0.1)))),
          Padding(
                ...
                Row(
                  children: [
                    ...
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Good morning,', style: TextStyle(color: Colors.white.withOpacity(0.6), fontSize: 13, fontWeight: FontWeight.w600)),
                        const Text('Ms. García', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 22)),
                        const SizedBox(height: 4),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                          decoration: BoxDecoration(color: AppColors.accentGreen.withOpacity(0.2), borderRadius: BorderRadius.circular(20), border: Border.all(color: AppColors.accentGreen.withOpacity(0.4))),
  ...
  Widget _buildAlertCard() {
    return Container(
      ...
      child: Row(
        children: [
          Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: AppColors.accentOrange.withOpacity(0.2), shape: BoxShape.circle), child: const Icon(Icons.warning_rounded, color: AppColors.accentOrange, size: 22)),
  ...
  Widget _buildClassOverview(List<StudentModel> students) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: AppColors.darkCard, borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.white.withOpacity(0.08))),
      child: Column(
        children: skills.map((s) => Padding(
          padding: const EdgeInsets.only(bottom: 12),
          child: Row(
            children: [
              SizedBox(width: 72, child: Text(s['label'] as String, style: TextStyle(color: Colors.white.withOpacity(0.7), fontWeight: FontWeight.w700, fontSize: 12))),
              Expanded(
                child: ClipRRect(
  ...
  Widget _buildStudentQuickCard(StudentModel student, BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(color: AppColors.darkCard, borderRadius: BorderRadius.circular(18), border: Border.all(color: Colors.white.withOpacity(0.06))),
        child: Row(
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text('Level ${student.level} · ${(student.completionPercent * 100).toInt()}% complete', style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 11)),
                ],
              ),
            ),
            Container(
  ...
  List<Widget> _buildActivityFeed() {
    final activities = [
      {'emoji': '✅', 'text': 'Sofia López completed World 2 Level 3', 'time': '10 min ago', 'color': AppColors.accentGreen},
      {'emoji': '🎯', 'text': 'Carlos Mendez started Speaking Mission', 'time': '1h ago', 'color': AppColors.primaryBlue},
      {'emoji': '⚠️', 'text': 'Miguel Torres missed daily goal', 'time': '3h ago', 'color': AppColors.accentOrange},
    ...
    return activities.map((a) => Container(
      ...
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(color: AppColors.darkCard, borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.white.withOpacity(0.06))),
      child: Row(
        children: [
          Container(
            width: 40, height: 40,
            decoration: BoxDecoration(color: (a['color'] as Color).withOpacity(0.15), shape: BoxShape.circle, border: Border.all(color: (a['color'] as Color).withOpacity(0.3))),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(a['time'] as String, style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: 10)),
              ],
            ),
          ),
        ],
